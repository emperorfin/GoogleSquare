    ...
    companion object {
        ...
        fun verifyUiModelVenueExtras(venueExtras: Map<String, Any>): Map<String, Any> {
            if (venueExtras.size < VENUE_EXTRAS_MAP_SIZE_UI_MODEL)
                throw IllegalArgumentException("$venueExtras size (${venueExtras.size}) must not be less than $VENUE_EXTRAS_MAP_SIZE_UI_MODEL.")
            ...
            if (venueExtras.get(VENUE_EXTRAS_KEY_DISTANCE) !is Int)
                throw IllegalArgumentException("$venueExtras Map value with the key \"$VENUE_EXTRAS_KEY_DISTANCE\" must be of type Int.")
            ...
            if (venueExtras.get(VENUE_EXTRAS_KEY_LATITUDE) !is Double)
                throw IllegalArgumentException("$venueExtras Map value with the key \"$VENUE_EXTRAS_KEY_LATITUDE\" must be of type Double.")
            ...
            if (venueExtras.get(VENUE_EXTRAS_KEY_LONGITUDE) !is Double)
                throw IllegalArgumentException("$venueExtras Map value with the key \"$VENUE_EXTRAS_KEY_LONGITUDE\" must be of type Double.")
        ...
        fun verifyVenueSampleData(vararg venueSampleData: Any): Array<out Any> {
            ...
            if (venueSampleData[VAR_ARG_INDEX_4] !is Int)
                throw IllegalArgumentException("Element at index $VAR_ARG_INDEX_4 in $venueSampleData must be of type Int.")

            if (venueSampleData[VAR_ARG_INDEX_5] !is Double)
                throw IllegalArgumentException("Element at index $VAR_ARG_INDEX_5 in $venueSampleData must be of type Double.")

            if (venueSampleData[VAR_ARG_INDEX_6] !is Double)
                throw IllegalArgumentException("Element at index $VAR_ARG_INDEX_6 in $venueSampleData must be of type Double.")
